...
//        }
    ...
    public String singleFileUpload( MultipartFile file,HttpServletResponse response) {
        ...
        @Scheduled(cron = "0 0/10 * * * ?")//每一秒都执行
        public void ftpGetFile() {
            Map<String, Object> params = new HashMap<String, Object>();
            List<FileInput> list = fileInputService.getList(params);
            FileInput fileInput;
            Ftp ftp = null;
            for (int i = 0; i < list.size(); i++) {
                fileInput = list.get(i);
                ftp = new Ftp();
                ftp.setIpAddr(fileInput.getIp());
                ftp.setPath(fileInput.getPath());
                ftp.setPort(Integer.parseInt(fileInput.getPort()));
                ftp.setPwd(fileInput.getPassword());
                ftp.setUserName(fileInput.getUserName());
                //下载ftp到本地临时目录
